...
async function check_and_create_ipns_key(channelid) {
    try {
        const ipfs = await create_ipfs_client();
        ...
        if (key) {
            ...
            await ipfs.key.gen(channelid, { type: 'rsa', size: 2048 });
...
async function check_ipns_key(channelid) {
    try {
        const ipfs = await create_ipfs_client();
        const res = await ipfs.key.list();
        const key = res.find((key) => key.name === channelid);
