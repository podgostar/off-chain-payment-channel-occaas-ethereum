...
const main = async (channel_id, sender, sender_pk, amount) => {
    ...
    try {
        ...
        // Simulating "that oracle event listener failed, and client is trying to reclaim the token"
        const tx = await contractInstance.joinChannel(join_token_encoded, { from: sender, value: amount });
        console.log('Transaction hash:', tx.tx);

        const reclaim_oracle = await oracle_channel.join_reclaim(join_token_encoded);

        if (!reclaim_oracle) {
            console.log('Join token reclaim failed!')
            return new Error('Join token reclaim failed')
        }
